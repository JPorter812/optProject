...
def main():
    ...
    model_runner = Model_runner(
        SimpleExponential(*parameters), timestep, start_time, end_time
    )
    ...
    perturbeddata = LinspaceRawData(
        perturbdata(modelResult.values, 0.05), timestep, start_time, end_time
    )
    ...
    model_fitter = Model_fitter(
        lambda parameters: SimpleExponential(
            A=parameters[0], B=parameters[1], y0=parameters[2]
        ),
        perturbeddata,
    )
    ...
    fittedModel = SimpleExponential(
        A=fittedparams[0], B=fittedparams[1], y0=fittedparams[2]
    )
...
def perturbdata(theoreticaldata, proportional_uncertainty):
    return theoreticaldata + np.random.normal(
        loc=0,
        scale=proportional_uncertainty * theoreticaldata,
        size=len(theoreticaldata),
    )
