    ...
    def optimise(self, objective_function, optalgo: str, options: dict): 
        ...
        if optalgo in ['nelder-mead', 'powell', 'cg', 'bfgs', 'l-bfgs-b', 'tnc', 'cobyla', 'slsqp', 'trust-constr']: 
            ...
            match optalgo: 
                ...
                case "basinhopping": 
                    optresult = scipy.optimize.basinhopping(objective_function,
                                                            x0 = options.get("startguess"),
                                                              niter = self._optimiser_configuration.iterations)
                case "dual_annealing": 
                    optresult = scipy.optimize.dual_annealing(objective_function, bounds=options.get("bounds"),
                                                               maxiter=self._optimiser_configuration.iterations)
                case "pso": 
                    boundslist = options.get("bounds")
                    solution, best_eval = pso(func=objective_function, 
